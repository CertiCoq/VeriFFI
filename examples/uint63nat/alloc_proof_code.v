 ...
 *)
 ...
 (* KS: During development - delete later. *)
 Set Nested Proofs Allowed.

Require Import VeriFFI.examples.uint63nat.glue.

 (* KS: Fix this. *)
(* Require Export VeriFFI.verification.graphCRep. *)
From VeriFFI Require Import verification.graph_add.
(* KS: This is the new version of generated glue and might make problems in this proof. 
...
(* KS01: Where are these Compspecs made from? *)
...
(** C code that assigns the argument _arg into the array __argv with corresponding offset,
e.g. for nat/S:
 ...
 * ((unsigned long long * ) argv + 1) = arg0;
 *)
Definition f_alloc_assign (__argv : ident) (offset : Z) (_arg: ident) :=
(Sassign
          (Ederef
              (Ebinop Oadd 
                   (Etempvar __argv (tptr (talignas 3%N (tptr tvoid))))
                    (Econst_long (Int64.repr offset) tlong)
                    (tptr (talignas 3%N (tptr tvoid)))) 
              (talignas 3%N (tptr tvoid)))
          (Etempvar _arg (talignas 3%N (tptr tvoid)))).
            ...
            (Ssequence
              ...
              (Sassign
                ...
                (Ebinop Oadd (Etempvar _t'1 (tptr (talignas 3%N (tptr tvoid))))
                  (Econst_long (Int64.repr n) tlong) (tptr (talignas 3%N (tptr tvoid))))))
...
(** C code for all assignments of previous arguments,
  ...
  *)
Fixpoint f_alloc_assigns (__argv : ident) (z : Z) (args : list ident) :=
  match args with
  | nil => f_alloc_standard z
  | arg :: args' => Ssequence (f_alloc_assign __argv z arg) (f_alloc_assigns __argv (1 + z) args')
end.

(** C code for the whole function *)
Definition f_alloc_make_general (__argv : ident) (args: list ident) (tag : Z) := {|
  fn_return := (talignas 3%N (tptr tvoid));
  fn_callconv := cc_default;
  fn_params := ((__tinfo, (tptr (Tstruct _thread_info noattr))) ::
                map (fun x => (x, (talignas 3%N (tptr tvoid)))) args );
  fn_vars := nil;
  ...
  (Sset __argv
    (Efield
      (Ederef (Etempvar __tinfo (tptr (Tstruct _thread_info noattr)))
        (Tstruct _thread_info noattr)) _alloc (tptr (talignas 3%N (tptr tvoid)))))
  ((Ssequence
  (Sassign
    (Ederef
      (Ebinop Oadd (Etempvar __argv (tptr (talignas 3%N (tptr tvoid)))) 
        (Econst_long (Int64.repr 0) tlong) (tptr (talignas 3%N (tptr tvoid)))) (talignas 3%N (tptr tvoid)))
    (Econst_long (Int64.repr tag) tlong)
    )
    (f_alloc_assigns __argv 1 args))))
|}.

(** This one is definitionally equal to the original code: *)

(* KS: cons is not found is this one. I want to uncomment this later on. *)

(* Lemma f_alloc_cons_eq1: f_alloc_make_general __argv [__arg0; __arg1] 2048 = f_alloc_make_Coq_Init_Datatypes_list_cons.
Proof. autounfold. reflexivity. Qed. *) 

Goal f_alloc_make_general __argv [__arg0] 1024 = f_alloc_make_Coq_Init_Datatypes_nat_S.
Proof. autounfold. 
  unfold f_alloc_make_general. reflexivity.
Qed.
...
Definition  n_arguments (tag : Z) (n : nat) : funspec :=
  ...
  PRE  [[ thread_info :: repeat (talignas 3%N (tptr tvoid)) n ]]
...
Ltac simplify_func_tycontext_ks' DD :=
  match DD with context [(func_tycontext ?f ?V ?G ?A)] =>
    ...
    (* KS: This had to change. It was previously: 
    pose (DS := @abbreviate (PTree.t funspec) DS1); but Ptree.t does no longer exist.
    Same problem with ptree_set. 
    I'll want to see how this is in VST at the moment.
    *)
(* KS    pose (DS := @abbreviate (PTree.t funspec) DS1); *)
    change (make_tycontext_s G) with DS in Delta;
    hnf in DS1;
    cbv beta iota delta  (* KS [ptree_set] *) in DS1;
    subst DS1;
...
Ltac repeat_forward tac :=
  ...
  (* Check these are the CertiGC compspecs. *)
Lemma alloc_make_nat_general_proof1 _alloc_f (t: Z) : 
  semax_body Vprog Gprog (f_alloc_make_general __argv (make_args 1) t) (_alloc_f, n_arguments t 1).
Proof.
...
Lemma alloc_make_nat_general_proof2  _alloc_f (t: Z) : semax_body Vprog Gprog (f_alloc_make_general __argv (make_args 2) t) (_alloc_f, n_arguments t 2).
Proof.
    (* KS: No longer applicable. *)
(*   alloc_start_function.
  repeat_forward custom_tactics.
Qed. *) 
Admitted.
...
(*
Lemma alloc_make_nat_general_proof3 _alloc_f (t: Z) : semax_body Vprog Gprog (f_alloc_make_general __argv (make_args 3) t) (_alloc_f, n_arguments t 3).
Proof.
  alloc_start_function.
   repeat_forward custom_tactics.
Qed.

Lemma alloc_make_nat_general_proof4 _alloc_f (t: Z) : semax_body Vprog Gprog (f_alloc_make_general __argv (make_args 4) t) (_alloc_f, n_arguments t 4).
Proof.
  alloc_start_function.
  repeat_forward custom_tactics.
Qed.
...
Lemma alloc_make_nat_general_proof5 _alloc_f (t: Z) : semax_body Vprog Gprog (f_alloc_make_general __argv (make_args 5) t) (_alloc_f, n_arguments t 5).
Proof.
  alloc_start_function.
  repeat_forward custom_tactics.
Qed. *)
...
(* KS: nat_in_graph doesn't exist at the moment... 
I have to download graphCRep.v for this. Or ask Joomy how this is generated. 
...
(* KS get_fields is in proofs_library_general.v 
...
(* KS: Deifnition of representable is changed. *)

(* Lemma add_node_monotone_list X (X_rep : Rep X) g to lb e n p:
 add_node_compatible g (new_copied_v g to) e -> graph_has_gen g to ->
  list_in_graph (X_in_graph X_rep) g n p -> 
  list_in_graph (X_in_graph X_rep) (add_node g to lb e) n p.
Proof.
...
(* Goal  forall  (g : graph) (descr : constructor_description)
 (t: nat) (ps : list rep_type) (args_my : specs_general.args (constr descr)) ,
    graph_has_gen g 0 ->
    forall (R1 : 0 <= Z.of_nat t < 256)
      (R2 : 0 < Zlength (map rep_field ps) < two_power_pos 54)
      (R3 : NO_SCAN_TAG <= Z.of_nat t -> ~ In None (map rep_field ps)),
...
(* KS: the general definition of constructor_description is broken. *)
...
(* KS: X_in_graph_cons - what is this now? *)
(* KS: Is the definition of alloc_make_spec_general the same as before or has it been changed? - URGENT *)
Definition geneeral_subsumption t n descr :
  (* KS: Remove - but how to get this back? **)
  X_in_graph_cons descr t ->  
  0 <= Z.of_nat t < 250 ->
  (* n = get_size (constr descr) margs -> *)
  0 < Z.of_nat n < 2096895 ->
  funspec_sub (n_arguments (calc t n) n) (* KS: (snd *) (alloc_make_spec_general descr n) (* ) *).
Proof.
(* KS - Removed as X_in_graph doesn't seem to exist.  *)
...
(* KS: Add in again. 
  ...
  assert (~ In (inr (new_copied_v gr 0)) roots) as ROOTS_IN by 
  (* KS: back in. eapply new_node_roots; eapply H). *) admit.
  ...
  { subst; try eapply spaces_g0; eauto. (* KS: Back in again. *) admit. }
  (* KS: Add in again.
  
    simpl in H2. injection H2. intros. subst x. clear H2. *)

  (* Ensuring the position of the pointer. *)
  ...
  Exists (spatial_gcgraph.outlier_rep outliers * ti_token_rep tinfo *
...
Proof.
  ...
  assert (__argv = _alloc_make_Coq_Init_Datatypes_list_cons) as -> by admit.
  (* assert (__argv = c *)
