...
Fixpoint in_graphs (g : graph) (c: constrCode) (xs :args c) (ps : list rep_type) : Prop :=
      match c as l return (args l -> Prop) with
      | TYPEPARAM f => fun H => let (X_, xs') := H in in_graphs g (f X_) xs' ps
      | PARAM X f => fun H => let (x, xs') := H in in_graphs g (f x) xs' ps                                                             
      | INDEX X_ f => fun H => let (x, xs') := H in
                          match ps with
                          | [] => False
                          | p :: ps' =>  X_in_graph X_ g x p  /\ in_graphs g (f x) xs' ps'
                          end     
      | ARG X_ arg => fun H : args (ARG X_ arg) =>
                      let (x, xs') := H in
                      match ps with
                      | [] => False
                      | p :: ps' => X_in_graph X_ g x p /\ in_graphs g arg xs' ps'
                      end  
     | DEPARG X_ f => fun H  => let (x, xs') := H in 
                  match ps with
                          | [] => False
                          | p :: ps' =>  X_in_graph X_ g x p  /\ in_graphs g (f x) xs' ps'
                          end                                                                             
...
(** List of tulong depending on the number of arguments which are represented in memory, needed for the paramers *)

Fixpoint get_size (c : constrCode) (xs : args c) : nat := 
   match c as l return (args l -> nat) with
      | TYPEPARAM f => fun H => let (X_, xs') := H in get_size (f X_) xs'
      | PARAM X f => fun H => let (x, xs') := H in get_size (f x) xs'                                                         
      | INDEX X_ f => fun H => let (x, xs') := H in S (get_size (f x) xs')    
      | ARG X_ arg => fun H : args (ARG X_ arg) =>
                      let (x, xs') := H in S (get_size arg xs')
     | DEPARG X_ f => fun H  => let (x, xs') := H in S (get_size (f x) xs')                                                   
      | RES x => fun _ => O                                                                            
      end xs.

Definition get_tulongs (c: constrCode) (xs: args c): list type :=
  repeat tulong (get_size c xs).

Lemma in_graphs_size g c xs ps : 
  in_graphs g c xs ps -> get_size c xs = length ps. 
Proof.
  revert ps. induction c; simpl in *; intros; try (destruct xs); eauto. 
  all: destruct ps; simpl; intuition eauto; congruence. 
Qed.
...
Definition alloc_make_spec_general (c : constructor_description) (n: nat) : ident * funspec :=
DECLARE (name c)
        WITH gv : globals, g : graph, ps : list rep_type, xs : args (constr c), roots : roots_t, sh : share, ti : val, outlier : outlier_t, f_info : fun_info, t_info : GCGraph.thread_info
  PRE  [[ thread_info :: repeat tulong n ]]
       PROP (n = get_size (constr c) xs;
            in_graphs g _ xs ps;
             writable_share sh
             )
  ...
  POST [ tptr tulong  ]  
     ...
     PROP ( X_in_graph (getRes (constr c) xs) g' (coqConstr c xs) p'; 
