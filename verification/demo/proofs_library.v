...
Lemma in_valid_RepNode_X (X : representable_X) g x p_x: 
  specs.X_in_graph X g x p_x -> valid_repNode g p_x.
Proof. 
  intros H. 
...
Proof.
  ...
  intros CS v p Tsh n A.
   ...
   destruct p0; entailer!.
   if_tac; eauto.
   + Search "||". apply orp_left. 
     * apply orp_right1.
       entailer. rewrite prop_true_andp. 
       unfold res_predicates.address_mapsto.
       simpl. destruct Nveric. simpl.
       Check predicates_hered.exp_left. 
       Print predicates_hered.derives.
(*      * Set Printing All. simpl. entailer. subst HH. rewrite HH.
   2: { admit. } 
   ...
   apply aggregate_pred.rangespec_ext.
   intros. 
   rewrite !at_offset_eq.
   destruct p; try contradiction. simpl.
   if_tac; eauto.
   +  admit. (* TODO : is_long and is_pointer stuff don't get true. *) 
   + unfold tc_val'.  simpl. 
     Print is_long.  (* Only true for Vlong _ *)
     Print is_pointer_or_integer. (* Archi.ptr64 -> also for long, otherwise also Vptr *) 
...
(* 
Lemma data_at_int_or_ptr_ptr_tulong:
 forall {CS: compspecs}  v p,
Archi.ptr64 = true -> data_at Tsh int_or_ptr_type v p |--
  data_at Tsh (tptr tulong) v p
 .
Proof.
  intros CS v p A.
   ...
   + unfold tc_val'.
   unfold tc_val; simpl. entailer!.
   unfold is_long in *. unfold is_pointer_or_integer.
   exfalso.
   destruct v; try ( eauto; congruence).
