...
From VC Require Export proofs_library subsumptiontests.
...
(* TODO: start_function does not work with the general version *) 
Lemma alloc_make_nat_S_proof (* args t *) : semax_body Vprog Gprog (f_alloc_make_general [_arg0] 1024 (* args t *) ) (_alloc_make_Coq_Init_Datatypes_nat_S, n_arguments 1024 1).
Proof.
  start_function.
  Compute (reptype thread_info). 
  Print thread_info.
  (* thread_info = tptr thread_info_type *) 
  Print thread_info_type.
  (* thread_info_type = Tstruct _thread_info noattr
     : type *) 
  Compute (reptype thread_info_type).
  assert (v : (val * (val * (val * list val)))%type) by admit.
  replace (field_at sh tulong [StructField _alloc] alloc ti) with (data_at sh thread_info_type v ti).
  destruct v as (myalloc&limit&bla&heap).
  forward. 
  - entailer!. (* TODO: is_pointer_or_null myalloc *) 
    admit. 
  - forward. 
    entailer!. (* TODO: is_pointer_or_null limit *) 
    admit. 
    forward.
    assert (b : block) by admit.
    assert (alloc_offset : Z) by admit.
    assert (limit_offset : Z) by admit.
    assert (myalloc = Vptr b (Ptrofs.repr alloc_offset)) by admit. 
    assert (limit = Vptr b (Ptrofs.repr limit_offset)) by admit. 


    forward_if (alloc_offset - limit_offset < 3). (* TODO: Right condition. *) 
   + entailer!. admit. 
   + admit. 
   + forward. 
     admit. 
...
(* TODO: start_function does not work with the general version *) 
Lemma alloc_make_nat_general_proof args t : semax_body Vprog Gprog (f_alloc_make_general args t ) (_alloc_make_Coq_Init_Datatypes_nat_S, n_arguments t (length args)).
Proof. 
  start_function.
  (* TODO: Would have to generalize this. Has something like this been used in another proof? *) 
